...
const JournalPrint = forwardRef<HTMLDivElement, JournalPrintProps>(({ journalData, currentPage = 1, totalPages = 1 }, ref) => {
        ...
        >
            ...
            <Box sx={{ textAlign: 'center', mb: 3 }}>
                ...
                <Typography variant="body2" sx={{ color: '#555', mt: 0.5 }}>
                    Branch: {journalData.branch_code || 'Main Branch'} | Date & Time: {getCurrentDateTime()}
                </Typography>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                <Typography variant="body1">
                    <strong>Voucher No:</strong> {journalData.journal_id}
                </Typography>
                <Typography variant="body1">
                    <strong>Date:</strong> {formatDate(journalData.journal_date)}
                </Typography>
            </Box>

            <Box sx={{ mb: 2 }}>
                <Typography variant="body1">
                    <strong>Debit From (A/c):</strong> {journalData.debit_from || 'N/A'}
                </Typography>
            </Box>

            {journalData.credit_to && (
                <Box sx={{ mb: 2 }}>
                    <Typography variant="body1">
                        <strong>Credit To (A/c):</strong> {journalData.credit_to}
                    </Typography>
                </Box>
            )}
            ...
            <Box sx={{ mb: 2 }}>
                <Typography variant="body1">
                    <strong>Narration:</strong> {journalData.journal_details || '-'}
                </Typography>
            </Box>

            <Box sx={{ mb: 2 }}>
                <Typography variant="body1">
                    <strong>Entry Type:</strong> {journalData.mode_of_entry || 'Journal'}
                </Typography>
            </Box>
            ...
            <Box sx={{ mb: 3, p: 2, bgcolor: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 1 }}>
                <Typography variant="h6" sx={{ fontWeight: 'bold', color: '#0f172a' }}>
                    Amount: ₹{Number(journalData.amount || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </Typography>
                <Typography variant="body2" sx={{ fontStyle: 'italic', mt: 0.5, color: '#475569' }}>
                    Amount in words: {convertAmountToWords(Number(journalData.amount || 0))}
                </Typography>
            </Box>
...
JournalPrint.displayName = 'JournalPrint';
